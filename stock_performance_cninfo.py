...
def upsert_performance(df):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS stock_performance (
            ...
            code varchar not null comment '证券代码',
            name varchar comment '证券简称',
...
def stock_forcast(url='http://www.cninfo.com.cn/data20/performanceForecast/list?rdate=20230331', data={}):
    headers = {
...
def upsert_forcast(df):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS stock_forcast (
            ...
            code varchar not null comment '证券代码',
            name varchar comment '证券简称',
            industry_2nd varchar comment '二级行业',
            perform_type varchar comment '业绩变动类型',
            forcast_content varchar comment '业绩变动原因',
            perform_reason varchar comment '业绩变动幅度',
...
cursor.execute("""
    CREATE TABLE IF NOT EXISTS stock_forcast (
        ...
        code varchar not null comment '证券代码',
        name varchar comment '证券简称',
        update_date date comment '公告日期',
        industry_2nd varchar comment '申万二级行业',
        perform_type varchar comment '业绩类型',
        forcast_content varchar comment '业绩预告内容',
        perform_reason varchar comment '业绩变化原因',
        net_profit_upper float comment '净利润增长幅上限(%)',
        net_profit_lower float comment '净利润增长幅下限(%)',
        PRIMARY KEY (report_date, code)
    )
""")
